...
Now we need to index the reference sequence using Minimap2
```bash
minimap2 \
-x map-ont \
...
Thus we need to add the option `-a` to obtaine an output in sam format
```bash
# Rerun the alignment
minimap2 \
-a \
...
### Convert the sam to a sorted bam file 
{: .no_toc}
...
# Index the bam file 
samtools index -@ ${threads} ont/alignments/rkatsiteli.leaves.ont.sort.bam
...
-l chr05:24461000-24462000 wgbs/alignments/rkatsiteli.leaves_pe.deduplicated.sort.bam ont/alignments/rkatsiteli.leaves.ont.sort.bam \
-n wgbs,ont
```
...
For example to set a threshold for cytosine modifications at 0.8 a provide `--filter-threshold C:0.8`.
